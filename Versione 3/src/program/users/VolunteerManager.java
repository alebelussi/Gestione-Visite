...
public class VolunteerManager {
	...
	public Volunteer remove() {
		...
		if(volunteerList.isEmpty()) {
			System.out.println("Attenzione: non ci sono volontari disponibili!");
			return null;
		}

		do {
			do {
				nickname = InputDati.leggiStringaNonVuota("Inserisci il nickname del volontario: ");
				
				if(!volunteerList.findVolunteer(nickname))
					System.out.println("Errore...volontario non presente!");
			}while(!volunteerList.findVolunteer(nickname));
			
			confirmInsert = InputDati.yesNo("Vuoi rimuovere il volontario?");
			if(confirmInsert) {
